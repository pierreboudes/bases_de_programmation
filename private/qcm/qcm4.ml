...
let nb_qcm = 60  (* Nombre de QCM à générer *)
let nb_ques = 15  (* nombre de questions par QCM *)

let input = "questions4.tex" (* fichier d'entrée *)
...
let is_valid qcm =  ((List.length (List.filter dif1 qcm))>=4) 
                    && ((List.length (List.filter dif1 qcm))<=5) 
                    && ((List.length (List.filter facile1 qcm))>=5)
...
\\vspace{-.2cm}
DUT R\\&T \\hfill {{\\Large\\bf Algorithmique et programmation -- contrôle continu}} \\hfill 
...
%\\raggedcolumns
\\noindent
\\textbf{\\small Barème du QCM~: $1$ points par réponse juste (unique) ; $-0,5$ points par réponse fausse.}
...
\\begin{multicols}{2}
\\paragraph{Premier exercice sur feuille (2,5 pt).} Écrire un programme qui demande à l'utilisateur de saisir trois entiers et trouve et affiche le minimum de ces trois entiers. Vous pouvez (sans y être obligés) utiliser des fonctions utilisateur, que vous devrez alors correctement déclarer et initialiser.

\\paragraph{Second exercice sur feuille (2,5 pt).} Déclarer et définir une procédure qui prend en entrée un entier \\texttt{longueur} et affiche une ligne de \\texttt{longueur} étoiles suivie d'un saut de ligne. En faisant uniquement appel à cette procédure pour les affichages, écrire une fonction principale qui dessine un carré d'étoiles de cent étoiles de côté. 
\\end{multicols}
\\newpage
\\Huge FIN
\\end{document}
