...
let nb_qcm = 1  (* Nombre de QCM à générer *)
let nb_ques = 66  (* nombre de questions par QCM *)
...
\\paragraph{Second exercice sur feuille (2,5 pt).} Déclarer et définit une procédure qui prend en entrée un entier longueur et affiche une ligne de longueur étoiles suivie d'un saut de ligne. En faisant uniquement appel à cette procédure pour les affichages, écrire une fonction principale qui dessine un carré d'étoiles de côté 100. 
\\end{multicols}
\\newpage
\\Huge FIN
\\end{document}
